d3 = function() {
  ...
  var PI = Math.PI, ε = 1e-6, d3_radians = PI / 180, d3_degrees = 180 / PI;
  function d3_sgn(x) {
  ...
  function d3_geo_clipAreaRing(ring, invisible) {
    ...
    while (++i < n) {
      ...
      if (Math.abs(y) < ε || Math.abs(y0) < ε) {} else if (Math.abs(Math.abs(x - x0) - PI) < ε) {
        if (y + y0 > 2) area += 4 * (x - x0);
      } else if (Math.abs(y0 - 2) < ε) area += 4 * (x - x1); else area += ((3 * PI + x - x0) % (2 * PI) - PI) * (y0 + y);
      x1 = x0, x0 = x, y0 = y;
    }
    return area;
  }
  ...
  function d3_geo_clipAntimeridianLine(listener) {
    ...
    return {
      ...
      point: function(λ1, φ1) {
        var sλ1 = λ1 > 0 ? PI : -PI, dλ = Math.abs(λ1 - λ0);
        if (Math.abs(dλ - PI) < ε) {
          listener.point(λ0, φ0 = (φ0 + φ1) / 2 > 0 ? PI / 2 : -PI / 2);
  ...
  function d3_geo_clipCircle(radius) {
    ...
    function intersect(a, b, two) {
      ...
      var δλ = λ1 - λ0, polar = Math.abs(δλ - PI) < ε, meridian = polar || δλ < ε;
      if (!polar && φ1 < φ0) z = φ0, φ0 = φ1, φ1 = z;
      if (meridian ? polar ? φ0 + φ1 > 0 ^ q[1] < (Math.abs(q[0] - λ0) < ε ? φ0 : φ1) : φ0 <= q[1] && q[1] <= φ1 : δλ > PI ^ (λ0 <= q[0] && q[0] <= λ1)) {
