...
(function($) {
    ...
    var
        defaults = {
            // The default output mode is label/Layer because it requires no dependencies.
            outputMode:"Layer",
    ...
    $.validity = {
        ...
        patterns:{
            ...
            eng:/^[a-zA-Z]*$/,
            eng_num:/^[a-zA-Z0-9]*$/,
            eng_num_email:/^[a-zA-Z0-9.]*$/,
            kor:/^[가-힣]*$/,
            eng_num_han:/^[a-zA-Z0-9一-龥]*$/,
            eng_num_kor:/^[a-zA-Z0-9가-힣]*$/,
            eng_num_kor_han:/^[a-zA-Z0-9가-힣一-龥]*$/,
            jumin:/\d{6}(\-|)[1-4]\d{6}$/,
            ...
            mphonNum : /^[0]\d{2}-(\d{3}|\d{4})-\d{4}$/,
            ynFlag:/^[yYnN]$/,
            guid:/^(\{?([0-9a-fA-F]){8}-(([0-9a-fA-F]){4}-){3}([0-9a-fA-F]){12}\}?)$/,
            time12:/^[01]?\d:[0-5]\d?\s?[aApP]\.?[mM]\.?$/,			/* am/pm */
        ...
        messages:{
            require:"#{field} 항목은 꼭 입력해야 합니다.",
            // Format validators:
            jumin:"#{field} 항목의 형식(주민번호)이 올바르지 않습니다.",
            match:"#{field} 항목의 형식이 올바르지 않습니다.",
            integer:"#{field} 항목은 정수형으로 입력해야 합니다.",
            date:"#{field} 항목의 형식(날짜)이 올바르지 않습니다.",
            email:"#{field} 항목의 형식(이메일)이 올바르지 않습니다.",
            usd:"#{field} 항목의 형식이 올바르지 않습니다.",
            url:"#{field} 항목의 형식(URL)이 올바르지 않습니다.",
            number:"#{field} 항목은 숫자만 입력해야 합니다.",
            zip:"#{field} 항목의 형식(우편번호)이 올바르지 않습니다.",
            phone:"#{field} 항목의 형식(전화번호)이 올바르지 않습니다.",
            mphonNum:"#{field} 항목의 형식(전화번호)이 올바르지 않습니다.",
            ynFlag:"#{field} 항목은 Y, 또는 N 값만 입력해야 합니다.",
            guid:"#{field} 항목의 형식(GUID)이 올바르지 않습니다.",
            time24:"#{field} 항목의 형식이 올바르지 않습니다.",
            time12:"#{field} 항목의 형식이 올바르지 않습니다.",

            // Value range messages:
            lessThan:"#{field} 항목은 #{max} 보다 작은 값을 입력해야 합니다.",
            lessThanOrEqualTo:"#{field} 항목은 #{max} 이하의 값을 입력해야 합니다.",
            greaterThan:"#{field} 항목은 #{min} 보다 큰 값을 입력해야 합니다.",
            greaterThanOrEqualTo:"#{field} 항목은 #{min} 이상의 값을 입력해야 합니다.",
            range:"#{field} 항목은 #{min} 이상, #{max} 이하의 값을 입력해야 합니다.",

            // Value length messages:
            tooLong:"#{field} 항목은 최대 #{max} 자 까지만 입력할 수 있습니다.",
            tooShort:"#{field} 항목은 최소 #{min} 자 이상 입력해야 합니다.",
            tooLongByte:"#{field} 항목은 최대 #{max} 바이트(bytes) 까지만 입력할 수 있습니다.",
            tooShortByte:"#{field} 항목은 최소 #{min} 바이트(bytes) 이상 입력해야 합니다.",

            // Aggregate validator messages:
            equal:"값이 일치 하지 않습니다.",
            distinct:"중복된 값이 존재합니다.",
            sum:"Values don't add to #{sum}.",
            sumMax:"The sum of the values must be less than #{max}.",
            sumMin:"The sum of the values must be greater than #{min}.",

            nonHtml:"#{field} 항목에는 HTML 을 입력할 수 없습니다.",
    ...
    $.fn.extend({
        ...
        maxByteLength:function(max, msg) {
            return validate(
                this,
                function(obj) {
                    return getByteLength(obj.value) <= max;
                },
                msg || format(
                    $.validity.messages.tooLongByte, {
                        max:max
                    }
                )
            );
        ...
        minByteLength:function(min, msg) {
            return validate(
                this,
                function(obj) {
                    return getByteLength(obj.value) >= min;
                },
                msg || format(
                    $.validity.messages.tooShortByte, {
                        min:min
                    }
                )
            );
        ...
        jumin:function(msg) {
            return validate(
                this,
                function(obj) {
                    return CheckSocialNumber(obj.value);
                },
                msg || format(
                    $.validity.messages.jumin, {
                        max:max
                    }
                )
            );
    ...
    function CheckSocialNumber(socialNo) {
    	var answer = 0;
    	var i = 2;
    	for(var a=0;a<socialNo.length-1;a++) {
    		answer+=parseInt(socialNo.charAt(a))*i;
    		i++;
    		if(i>9) i=2;
    	}
    	return ((11-(answer%11))==socialNo.charAt(socialNo.length-1));
    }
